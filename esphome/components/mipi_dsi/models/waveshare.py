...
DriverChip(
    ...
    lane_bit_rate="1Gbps",   # 2 lanes total ≈ 1.0–1.2 Gbps works well
    swap_xy=cv.UNDEFINED,
    color_order="RGB",
    initsequence=[
        # Sleep Out + wait
        (0x11,),
        # === Page selects / power / gamma (condensed working set) ===
        (0xFF, 0x98, 0x81, 0x00),
        (0x36, 0x00),              # MADCTL default; try 0x02 if orientation is wrong
        (0x3A, 0x55),              # 16-bit RGB565 pixel format  (0x77 would be 24-bit RGB888)
        (0xFF, 0x98, 0x81, 0x01),
        (0x00, 0x47), (0x01, 0x22), (0x02, 0x00), (0x03, 0x00),
        (0x04, 0x02), (0x05, 0x2A), (0x06, 0x15), (0x07, 0x21), (0x08, 0x0F),
        (0x0C, 0x1A), (0x0D, 0x1A), (0x0E, 0x3F), (0x0F, 0x3F),
        (0x10, 0x12), (0x11, 0x10), (0x12, 0x00), (0x13, 0x15),
        (0x14, 0x13), (0x15, 0x43), (0x16, 0x0B),

        (0xFF, 0x98, 0x81, 0x02),
        # Gamma (workhorse values that are known-good on the A panel)
        (0x00, 0x01), (0x01, 0x01), (0x02, 0x08), (0x03, 0x12), (0x04, 0x10),
        (0x05, 0x12), (0x06, 0x19), (0x07, 0x0C), (0x08, 0x13), (0x09, 0x06),
        (0x0A, 0x06), (0x0B, 0x0B), (0x0C, 0x0B), (0x0D, 0x10), (0x0E, 0x10),
        (0x0F, 0x10), (0x10, 0x10), (0x11, 0x00), (0x12, 0x08), (0x13, 0x12),
        (0x14, 0x10), (0x15, 0x12), (0x16, 0x19), (0x17, 0x0C), (0x18, 0x13),
        (0x19, 0x06), (0x1A, 0x06), (0x1B, 0x0B), (0x1C, 0x0B), (0x1D, 0x10),
        (0x1E, 0x10), (0x1F, 0x10), (0x20, 0x10),

        (0xFF, 0x98, 0x81, 0x03),
        # VCOM/VRG etc. (safe minimal set)
        (0x00, 0x01), (0x01, 0x01), (0x02, 0x00), (0x03, 0x00),
        (0x04, 0x04), (0x05, 0x1E), (0x06, 0x1E),
        (0x07, 0x00), (0x08, 0x85), (0x09, 0x85), (0x0A, 0x64),
        (0x0B, 0x64), (0x0C, 0x3C), (0x0D, 0x3C),

        # Back to user page
        (0xFF, 0x98, 0x81, 0x00),

        # Optional: DCS brightness path enabled and max brightness
        (0x53, 0x2C),
        (0x51, 0xFF),
